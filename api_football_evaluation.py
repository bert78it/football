...
class APIFootballDataSource:
    ...
    def fetch_matches(self, date=None, league_id=None):
        """Fetch football matches for a specific date and optional league"""
    ...
    def verify_api_access(self):
        ...
        try:
            ...
            response = requests.get(
                ...
                headers=self._get_headers(),
                params={'current': 'true'}  # Get current active leagues
            )
            ...
            if response.status_code == 200:
                ...
                for league in leagues[:10]:
                    logger.info(f"- {league.get('league', {}).get('name', 'Unknown')} (ID: {league.get('league', {}).get('id', 'N/A')})")
...
def main():
    api_source = APIFootballDataSource()
